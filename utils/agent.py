...
class Agent(FrozenClass):
    ...
    def load_data_and_initialization(self,tile_name):
        ...
        '''
        # load images
        self.sat_image = np.array(Image.open(os.path.join(self.args.dataroot,f'{tile_name}.png')))
            ...
            for i in range(self.image_size//self.crop_size+1):
                for j in range(self.image_size//self.crop_size+1):
                    ...
                    binary_mask[self.crop_size//2+i*self.crop_size:self.crop_size//2+(i+1)*self.crop_size,\
                            self.crop_size//2+j*self.crop_size:self.crop_size//2+(j+1)*self.crop_size] = pred_mask
                    point_mask[self.crop_size//2+i*self.crop_size:self.crop_size//2+(i+1)*self.crop_size,\
                            self.crop_size//2+j*self.crop_size:self.crop_size//2+(j+1)*self.crop_size] = pred_point
    ...
    def get_valid_coords(self,pred_logits,pred_coords,thr=0.5):
        ...
        for ii, coord in enumerate(pred_coords_ROI):
            if self.args.process_boundary and (coord[0]+self.current_coord[0]-self.crop_size//2<=75 \
