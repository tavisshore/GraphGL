...
from models.gnn.gbt.gssl.loss import barlow_twins_loss
from models.gnn.gbt.gssl.transductive_model import augment
from models.gnn.gbt.gssl.inductive.tasks import evaluate_node_classification


class Model:
    def __init__(self, feature_dim: int, emb_dim: int, p_x: float, p_e: float, lr_base: float, total_epochs: int, warmup_epochs: int):
        self._device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self._encoder = GATEncoder(in_dim=feature_dim, out_dim=emb_dim).to(self._device)
        self._optimizer = torch.optim.AdamW(params=self._encoder.parameters(), lr=lr_base, weight_decay=1e-5)
        self._p_x = p_x
        self._p_e = p_e
        self._total_epochs = total_epochs

    def fit(self, data: Dict[str, PPI], log_interval: Optional[int] = None) -> dict:
        self._encoder.train()
        logs = {"log_epoch": [], "train_f1": [], "val_f1": [], "test_f1": [], "z": []}
        train_loader = DataLoader(dataset=data["train"], batch_size=1, shuffle=True)
        ...
        for epoch in tqdm(iterable=range(self._total_epochs)):
            ...
            for batch in train_loader:
                ...
                (x_a, ei_a), (x_b, ei_b) = augment(data=batch, p_x=self._p_x, p_e=self._p_e)
    ...
    def _log(self, data, epoch, logger, logs):
        ...
        f1 = evaluate_node_classification(z_train=z_train, y_train=y_train, z_val=z_val, y_val=y_val, z_test=z_test, y_test=y_test)
        ...
        logs["z"].append((deepcopy(z_train), deepcopy(z_val), deepcopy(z_test)))
    ...
    def predict(self, data: PPI) -> torch.Tensor:
        ...
        with torch.no_grad():
            for batch in tqdm(loader, desc="Predict", leave=False):
                z = self._encoder(x=batch.x.to(self._device), edge_index=batch.edge_index.to(self._device))
                zs.append(z.cpu())
